...
class Mysql(object):
    ...
    def __getConn():
        ...
        if Config.POOL is None:
            Config.POOL = PooledDB(creator=MySQLdb, mincached=1 , maxcached=20 ,
...
if __name__ == "__main__":
    mysql = Mysql()
    ##服务器时区为UTC，转为当前时区+8小时
    dt=(datetime.datetime.now() + datetime.timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S")
    mysql.insertOne("insert into tbl_evaluate_tasks(gmt_create, gmt_modified, api, params, result) values(%s,%s,%s,%s,%s)", [dt, dt, api, params, result])
    mysql.dispose()